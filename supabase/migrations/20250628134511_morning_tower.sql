/*
  ...
  1. New Tables
    - `processing_metrics` - Track AI processing steps and performance
    - `cost_tracking` - Monitor costs across all services
    - `retention_policies` - Automated data cleanup policies
    - `system_health` - System-wide health metrics
    - `budget_alerts` - Cost monitoring and alerts

  2. Enhanced Audit Logging
    - Added session tracking, request IDs, and cost attribution
...
BEGIN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'audit_logs' AND column_name = 'cost_cents'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'audit_logs' AND column_name = 'processing_time_ms'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'audit_logs' AND column_name = 'provider_used'
  ) THEN
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_processing_metrics_report_step') THEN
    CREATE INDEX idx_processing_metrics_report_step 
    ON processing_metrics (report_id, step_name, created_at DESC);
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_cost_tracking_user_period') THEN
    CREATE INDEX idx_cost_tracking_user_period 
    ON cost_tracking (user_id, billing_period, created_at DESC);
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_cost_tracking_project_service') THEN
    CREATE INDEX idx_cost_tracking_project_service 
    ON cost_tracking (project_id, service_type, created_at DESC);
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_system_health_metric_time') THEN
    CREATE INDEX idx_system_health_metric_time 
    ON system_health (metric_name, recorded_at DESC);
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_budget_alerts_user_active') THEN
    CREATE INDEX idx_budget_alerts_user_active 
    ON budget_alerts (user_id, is_active, created_at DESC);
  END IF;
END $$;

-- Enhanced RLS policies (enable RLS first)
ALTER TABLE processing_metrics ENABLE ROW LEVEL SECURITY;
ALTER TABLE cost_tracking ENABLE ROW LEVEL SECURITY;
ALTER TABLE retention_policies ENABLE ROW LEVEL SECURITY;
ALTER TABLE budget_alerts ENABLE ROW LEVEL SECURITY;

-- System health doesn't need RLS (read-only for all authenticated users)
ALTER TABLE system_health ENABLE ROW LEVEL SECURITY;

-- Processing metrics policies (with existence checks)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'processing_metrics' 
    AND policyname = 'Users can read own processing metrics'
  ) THEN
    CREATE POLICY "Users can read own processing metrics"
...
-- Cost tracking policies (with existence checks)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'cost_tracking' 
    AND policyname = 'Users can read own costs'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'cost_tracking' 
    AND policyname = 'Users can insert own costs'
  ) THEN
...
-- Retention policies (with existence checks)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'retention_policies' 
    AND policyname = 'Users can manage own retention policies'
  ) THEN
...
-- Budget alerts policies (with existence checks)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'budget_alerts' 
    AND policyname = 'Users can manage own budget alerts'
  ) THEN
...
-- System health is read-only for authenticated users (with existence check)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'system_health' 
    AND policyname = 'Authenticated users can read system health'
  ) THEN
