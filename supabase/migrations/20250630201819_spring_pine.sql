/*
  ...
  1. New Tables
    ...
    - `retention_policies` - Data retention and cleanup policies
    - `system_health` - System health metrics and monitoring
    - `budget_alerts` - Budget monitoring and alerting

  2. Enhanced Audit Logging
    - Add session tracking, request IDs, cost tracking
    - IP address and user agent logging
    - Processing time and provider tracking
...
-- Drop existing policies if they exist and create new ones with unique names
DO $$
BEGIN
  ...
  DROP POLICY IF EXISTS "processing_metrics_user_read_v2" ON processing_metrics;
END $$;

CREATE POLICY "processing_metrics_user_read_v3"
...
BEGIN
  ...
  DROP POLICY IF EXISTS "cost_tracking_user_read_v2" ON cost_tracking;
  DROP POLICY IF EXISTS "cost_tracking_user_insert_v2" ON cost_tracking;
END $$;

CREATE POLICY "cost_tracking_user_read_v3"
  ON cost_tracking
  FOR SELECT
  TO authenticated
  USING (user_id = auth.uid());

CREATE POLICY "cost_tracking_user_insert_v3"
  ON cost_tracking
  FOR INSERT
  TO authenticated
...
BEGIN
  ...
  DROP POLICY IF EXISTS "retention_policies_user_all_v2" ON retention_policies;
END $$;

CREATE POLICY "retention_policies_user_all_v3"
...
BEGIN
  ...
  DROP POLICY IF EXISTS "budget_alerts_user_all_v2" ON budget_alerts;
END $$;

CREATE POLICY "budget_alerts_user_all_v3"
  ON budget_alerts
  FOR ALL
  TO authenticated
  USING (user_id = auth.uid())
...
BEGIN
  ...
  DROP POLICY IF EXISTS "system_health_authenticated_read_v2" ON system_health;
END $$;

CREATE POLICY "system_health_authenticated_read_v3"
  ON system_health
  FOR SELECT
  TO authenticated
  USING (true);
