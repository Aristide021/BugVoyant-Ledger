...
export class SlackService {
  ...
  private buildIncidentMessage(data: SlackNotificationData): SlackMessage {
    ...
    const features = [];
    if (data.hasAudio) {
      features.push('🎧 Audio Summary');
    }
    if (data.hasBlockchainProof) {
      features.push('⛓️ Blockchain Anchored');
    }
    features.push(`🤖 AI-Generated (${data.provider})`);
    ...
    if (data.processingTime || data.costCents) {
      ...
      if (data.processingTime) {
        metrics.push(`⏱️ ${data.processingTime}s processing`);
      }
      if (data.costCents) {
        metrics.push(`💰 $${(data.costCents / 100).toFixed(3)} cost`);
      }
    ...
    if (data.hasBlockchainProof && data.algorandTx) {
      actions.push({
        ...
        url: `https://explorer.algorand.org/transaction/${data.algorandTx}`,
        style: 'default'
      });
    }
    ...
    blocks.push({
      ...
      elements: [
        {
          type: 'mrkdwn',
          text: `Generated by BugVoyant-Ledger • Powered by Nodely Algorand Infrastructure`
        }
      ]
    });
